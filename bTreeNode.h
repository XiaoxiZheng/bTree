...
public:
    ...
    BTreeNode *find(int k);// a method to find a key in subtree rooted with this node, returns NULL if k is not present.
    int findKey(int k);// A function that returns the index of the first key that is greater or equal to k
    void insertNonFull(int k); // A helper method to insert a new key in the subtree rooted with this node. With the assumption that the node is not full
    ...
    void delete_key(int k);// A wrapper method to remove the key k in subtree rooted with this node.
    void removeFromLeaf(int idx);// A helper method to assist delete_key(int k) so that the key present in idx-th position in this leaf node is removed
    void removeFromNonLeaf(int idx);// A helper method to remove the key present in idx-th position in a non-leaf node

    int getPred(int idx);// A function to get the predecessor of the key- where the key is present in the idx-th position in the node
    int getSucc(int idx);// A function to get the successor of the key- where the key is present in the idx-th position in the node
